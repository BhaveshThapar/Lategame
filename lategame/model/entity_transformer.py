...
class EntityTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.poke_proj = nn.Linear(layout.pokemon_dim, d_model)
        self.move_proj = nn.Linear(layout.move_dim, d_model)
    ...
    def arch_config(self) -> dict[str, int]:
    ...
    def _tokenize(self, obs: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        poke_tok = self.poke_proj(poke_flat)
        move_tok = self.move_proj(move_flat)
