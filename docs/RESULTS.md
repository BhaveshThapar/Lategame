...
- **OU pivot — Build 5: canonical move-slot ordering — built + run; RED (slot order fixed and *verified*,
  ...
  - **Gate B1 (BC, before AWR spend) PASS — and the hypothesis's cheap prediction held.** Re-ingest needed
    ...
    verified). BC ET+prior val-acc **0.647 / 0.646 / 0.649** (mean 0.647 ± 0.002) — above the pre-registered 0.63
...
- **OU Pivot — Build 10: robustify pp by SYNTHESIZING full-pp deep-turn states — built + run; BC PASS but
  ...
  **deep-turn** (normalized turn ≥ threshold) rows, force the active mon's pp channels to full (present-guarded,
  ...
  - **Why (pp-reliance diagnostic, `results/pp_reliance_diag10.json`).** Self-contained pp-neutralization
    ...
    states, v10 baseline switch mass **0.597 ≈ v7 0.575** and v10 pp-**ΔP 0.390 ≈ v7 0.397** (neutralizing
    pp still collapses switch mass **0.597 → 0.206**) — the augmentation did **not** reduce pp-reliance at all. The synthesized examples are *real mid-game attack states with pp
...
- **OU Pivot — Build 20: the per-iteration sample budget — the plateau is a STATIONARY POINT, not a sampling-noise
  ...
  - **What the null teaches — the part worth keeping: 3× the samples collapsed seed-to-seed variance ~7× (std 0.074 →
    ...
    `results/grad_noise_diag_v20.json`, `results/ppo_ou_gate_v20.json`,
    `results/format_ceiling_gate_ou_v20.json`, `results/seed_strength_gate_v20.json` committed;
...
- **B6f STAGE A — 94.2% OF THE VGC SHARD CAME FROM 100 OF ITS 899 EPISODES, AND THE CAUSE WAS ONE
  ...
  - **SPLIT BY EPISODE LENGTH THE PICTURE INVERTS.** The non-loop episodes (≤40 turns) are 5,553
    ...
    0.571 and 2.62. Scored over decision turns alone the same shard reads **0.923** density and 14.0
    legal actions per slot, against OU's **0.9998** and 7.67. So VGC doubles is a normal,
    decision-DENSE format — denser per slot than OU — and the "98.4% of recorded turns had exactly
    one legal action per slot" note written
...
- **CLOSING THE OPEN LIST — TEAM PREVIEW MODELLED, THE VGC CEILING PROBE GETS ITS SECOND AND THIRD
  ...
  - **TEAM PREVIEW IS MODELLED, ON THE WHOLE FIELD, AND IT DOES NOT MOVE THE VERDICT.** Nothing
    ...
    (`results/format_ceiling_gate_vgc_v3.json`, `results/format_ceiling_gate_vgc_v3_nopreview.json`):
  ...
  - **M4 RUNS ON DOUBLES AS A BUILD, SIZED BY THE VERDICT.**
    ...
    **PASS** (160 episodes, 80/80, gap 6.86); curve vs `random` 0.950 → 0.983 and vs `heuristic`
    0.500 → 0.367; Gate C ladder
  ...
  - **THE RECORD'S TWO `vs heuristic` READS ARE NOT EVIDENCE ABOUT EACH OTHER, AND TWO GUESSES AT
    ...
    against one opponent range **0.367–0.550**:
