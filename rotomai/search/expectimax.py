...
from rotomai.features.action_space import action_mask
...
if TYPE_CHECKING:
    from rotomai.search.opponent_model import OpponentModel
...
def _advance_opp_pov(
    ...
    if opp_model is None or not getattr(opp_model, "needs_opp_pov", False) or live_opp is None:
        return None
    from rotomai.search.opponent_model import opp_pov_child
...
def _root_opp_pov(
    ...
    if opp_model is None or not getattr(opp_model, "needs_opp_pov", False):
        return None
    from rotomai.search.opponent_model import build_opp_pov
...
class PolicyValue:
    ...
    def greedy_order(self, battle: AbstractBattle) -> BattleOrder:
        ...
        from rotomai.features.action_space import action_to_order
