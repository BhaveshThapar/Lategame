...
the bias-robust metrics from ``rotomai.eval.rating`` ride alongside it. Against the fixed
...
from rotomai.agents.doubles_agent import DoublesAgent
from rotomai.agents.doubles_ppo_agent import DoublesPPORecordingAgent
...
from rotomai.config import DEFAULT_FORMAT, LOCAL_SERVER, is_doubles_format, local_account
from rotomai.eval.rating import gxe, rate_win_rate
...
def _singles_only_agents() -> set[str]:
    ...
    shaped-only mode (``ROTOMAI_SEARCH_SHAPED_ONLY=1``) the leaf is ``data.reward.state_value``,
...
class EvalResult:
    ...
    # field varies and win rate is pushed toward 50% by matchmaking. See rotomai/eval/rating.py.
...
#
# Set ROTOMAI_TEAM_PREVIEW=0 to restore poke-env's `random_teampreview` on both sides, which is
...
def team_preview_enabled() -> bool:
    return os.environ.get("ROTOMAI_TEAM_PREVIEW", "1") != "0"
...
def _with_team_preview(cls: type[Player], battle_format: str) -> type[Player]:
    ...
    if cls not in _PREVIEW_CACHE:
        from rotomai.agents.teampreview import TeamPreviewMixin
