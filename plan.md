...
- **G2.** Reach **strong-human performance** in the first target format, measured by matchmaking-bias-robust metrics (GXE / Glicko-1), competitive with the foul-play heuristic and Metamon baselines.
  ...
  - **The *ladder* half is now COMPUTED (2026-08-10), on an agent-only eval ladder rather than the human ladder.** GXE and Glicko-1 need a varied opponent field, which no amount of further training can supply — but §12's own last line prefers "a private/agent-only server or eval ladder", and §16 Q5 is answered: there is *no* unranked public ladder, so that route is the only policy-clean one. `rotomai/eval/ladder.py` fits a whole field jointly (Bradley-Terry on the Glicko scale, `heuristic` pinned at 1500). Over 8 agents × 28 pairs × 300 battles on `gen9ou`, **`v25b` reaches Glicko 1696.9 ± 14.9, GXE 0.6809**, above `simpleheuristics` (1579.2 / 0.5756) and the `heuristic` anchor (1500 / 0.5000) — see §13.1. Two caveats travel with it: the RD is a **lower bound** (battles cluster by team matchup, so the effective sample is under `n`), and this is an **agent-only** field, so the number is *not* comparable to a Showdown GXE measured against humans. **The human-ladder reading now EXISTS (2026-08-18), under the amended NG3.** One bounded, pre-registered run of 100 rated `gen9ou` games under a dedicated bot account returned **Elo 1004 / GXE 30% / Glicko 1339.5 ± 36.0**, a score rate of 0.350, and the pre-registered verdict **WEAK** — see §13.1 Build 29 and `results/live_ladder_gen9ou.json`. The two readings are not in tension and must not be averaged: the agent-only field says this checkpoint is far above `simpleheuristics`, and the human ladder says it is far below a median human. Both are measurements of what they measured, and the distance between them is the result — a bot-baseline win rate does not predict human-ladder performance. That is the evidence for prioritising the replay-data axis (§G3, and the last line of this bullet) over further self-play.
...
- **NG3.** Automated *farming* of the public *ranked* ladder — unattended, open-ended, or concurrent ranked play, play under a primary account, or play whose purpose is rating accumulation rather than a recorded measurement — remains out of scope (see Section 15 — policy-sensitive). **Amended 2026-08-18:** a single **bounded, pre-registered measurement run** is not farming and is in scope — one dedicated and clearly-labeled bot account, one battle at a time, paced between games, capped at an n fixed before the first game, played once, and reported whatever it returns including a bad result. The double opt-in gate (`--ladder-ack` + `ROTOMAI_LIVE_ALLOW_LADDER=1`) is unchanged, and the farming prohibition above is unchanged.
...
- **Measurement is not farming, and the difference is enforced in code rather than promised.** The one ranked activity in scope (NG3 as amended) is a bounded pre-registered read, and every constraint that makes it bounded is a refusal in `rotomai/live/`, not a habit: `--concurrency 1` (poke-env reads 0 as unlimited, and `build_live_player` refuses anything below 1), `--battle-delay` between games recorded into the results file, a fixed `--n`, `start_timer_on_battle_start` so no human waits on the clock, a login failure that is fatal and never retried so a bad password cannot be hammered, and bounded restarts with exponential backoff. The results file is rewritten after every battle, so the record cannot be curated after the fact.
- **Transparency:** prefer testing against opt-in opponents (challenges) and clearly-labeled environments. A ranked measurement run additionally requires the account name and profile to identify it as a bot and name this repository.
